#!/bin/bash
#SBATCH --job-name=ccf-level-profile
...
ROOT="${CCF_CACHE_ROOT}/runs/ccf_level_profile_job${SLURM_JOB_ID:?}"
mkdir -p "$ROOT"
...
sha256sum structured_utils.py structured_objective.py models/structured_decoder.py diffusion.py \
  ...
  scripts/profile_ccf_full_sample.py > "$ATTEMPT/code-sha256.txt"
srun --ntasks=1 python -u scripts/profile_ccf_full_sample.py --variant level_draws \
