...
# Continue all four matched CCF arms from update 3000 to update 6000.
# Validation and durable checkpoints occur every 500 updates: 3500, 4000,
# 4500, 5000, 5500, and 6000. Each arm resumes its full Lightning checkpoint,
# preserving optimizer, scheduler, RNG, and loop state.
...
# This activation form is retained from the successful continuation workflow.
cd "$CCF_CODE_ROOT"

CACHE_ROOT=${CCF_CACHE_ROOT}
SOURCE_ROOT="$CACHE_ROOT/runs/four_arm_continue_s001_k128_to3k_run-continue-3k"
...
RUN_ROOT="$CACHE_ROOT/runs/four_arm_continue_s001_k128_3k_to6k_${JOB_TAG}"
...
SOURCE_CHECKPOINT="$SOURCE_ROOT/$ARM/checkpoints/0-3000.ckpt"
...
if [[ -f "$RUN_DIR/checkpoints/last.ckpt" ]]; then
  ...
  echo "Restart $RESTART_COUNT: no continuation checkpoint exists; restarting this arm from update 3000"
fi
...
echo "Completed continuation through update 6000: $ARM"
