...
helpers={n:(w/n).read_text() for n in ['run_graph_intervention.py','graph_generation.py']}
remote='''
from pathlib import Path
import subprocess,json,hashlib
root=Path(ROOT);state=json.loads((root/'campaign.json').read_text())
assert 'graph_generation' not in state['jobs']
...
script=base+'python -u "$CCF_CAMPAIGN_ROOT/helpers/graph_generation.py" "$SLURM_ARRAY_TASK_ID"\\n'
(root/'graph_generation.sh').write_text(script)
cmd=['sbatch','--parsable','--array=0-3%1','--job-name=ccf15k-graph-generation','--partition=ALL','--time=04:00:00','--mem=36G','--cpus-per-task=4','--gres=gpu:1','--exclude=watgpu1008,watgpu1109,watgpu608,watgpu908','--mail-user=n23zhang@uwaterloo.ca','--mail-type=ALL,ARRAY_TASKS,TIME_LIMIT','--output='+str(root/'logs/graph-generation-%A_%a.out'),'--error='+str(root/'logs/graph-generation-%A_%a.err'),str(root/'graph_generation.sh')]
job=subprocess.check_output(cmd,text=True).strip().split(';')[0];assert job.isdigit();state['jobs']['graph_generation']=job
