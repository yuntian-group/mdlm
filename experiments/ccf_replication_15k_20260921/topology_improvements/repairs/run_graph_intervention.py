...
def head_forward(self,hidden_states,unary_logits,timestep,active_mask=None,**kwargs):
    hidden=hidden_states;unary=unary_logits
    global counter
...
save(trace.parent/'intervention.json',dict(intervention=kind,scope='Inference-only change to graph selection with checkpoint weights, vocabulary candidates, factor parameterization and scorer unchanged; the proposal candidate pool changes for active_chain_proposals. Raw pilot manifests identify the source checkpoint; this sidecar identifies the experimental graph intervention.',helper_sha256=sha(Path(__file__)),arguments_sha256=sha(args_file),trace_affects_timing=True))
